...
export function LoginPage() {
  ...
  return (
    <div className="w-full max-w-[440px]">
      ...
      <h2 className="mt-1 mb-6 text-2xl font-semibold text-foreground">Sign in to your workspace</h2>
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="space-y-2">
          ...
          {errors.companySlug && <p className="text-xs text-red-600 dark:text-red-400">{errors.companySlug.message}</p>}
        </div>
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" {...register("rememberMe")} className="accent-primary h-4 w-4 rounded" />
            Remember me
          </label>
          <Link to="/forgot-password" className="text-sm text-primary hover:underline font-medium">Forgot password?</Link>
        </div>
        {serverError && (
          <div className="flex items-start gap-2 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 px-4 py-3 text-sm text-red-700 dark:text-red-400">
