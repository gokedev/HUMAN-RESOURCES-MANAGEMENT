...
export const authService = {
    ...
    forgotPassword(payload) {
        return apiClient.post('/api/auth/forgot-password', payload).then((response) => response.data);
    },
    resetPassword(payload) {
        return apiClient.post('/api/auth/reset-password', payload).then((response) => response.data);
    },
    acceptInvitation(payload) {
        return apiClient.post('/api/auth/accept-invitation', payload).then((response) => response.data);
    },
};

export const employeeService = {
    // Fetches every page of the employee list so name maps and search work beyond a single page.
    async listAll() {
        const users = [];
        const size = 100;
    ...
    create(payload) {
        return apiClient.post('/api/admin/employees', payload).then((response) => response.data);
    },
    update(id, payload) {
        return apiClient.put(`/api/admin/employees/${id}`, payload).then((response) => response.data);
    },
    deactivate(id) {
        return apiClient.patch(`/api/admin/employees/${id}/deactivate`).then((response) => response.data);
    },
    reactivate(id) {
        return apiClient.patch(`/api/admin/employees/${id}/reactivate`).then((response) => response.data);
    },
};
...
export const departmentService = {
    ...
    delete(id) {
        return apiClient.delete(`/api/admin/departments/${id}`).then((response) => response.data);
    },
};
...
export const leaveService = {
    ...
    createMine(payload) {
        return apiClient.post('/api/employee/leave-requests', payload).then((response) => response.data);
    },
    listMine(params) {
        return apiClient
    ...
    cancelMine(id) {
        return apiClient.patch(`/api/employee/leave-requests/${id}/cancel`).then((response) => response.data);
    },
};
