...
import { FileEdit, LayoutDashboard, Users, ClipboardCheck, CalendarDays, UserRound, Menu, Moon, Sun, LogOut } from 'lucide-react';
...
const iconMap = {
    LayoutDashboard, Users, ClipboardCheck, CalendarDays, UserRound,
};
...
export function AppLayout() {
    ...
    // Mobile drawer state for the off-canvas sidebar navigation.
    ...
    return (<div className="app-shell">
      ...
      <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}>
        <Link to="/" className="brand-mark sidebar-brand" onClick={() => setSidebarOpen(false)}>
      ...
      <div className="app-main">
        ...
        <header className="topbar">
          <div className="topbar-context-group">
            <button className="icon-button menu-toggle" type="button" onClick={() => setSidebarOpen(true)} aria-label="Open navigation menu">
              <Menu size={18}/>
            </button>
            <div>
          ...
          <div className="topbar-actions">
            ...
            <button className="btn btn-outline-secondary btn-sm logout-btn" type="button" onClick={() => setShowLogoutConfirm(true)}>
      ...
      {showLogoutConfirm && (<ConfirmDialog title="Sign out" message="Are you sure you want to sign out of your workspace?" confirmLabel="Sign out" variant="danger" onConfirm={() => { setShowLogoutConfirm(false); logout(); }} onClose={() => setShowLogoutConfirm(false)}/>)}
    </div>);
}
